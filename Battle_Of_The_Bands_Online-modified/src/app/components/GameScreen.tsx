...
export function GameScreen({ teams, currentTurn, currentTurns, teamSize, battleKey, bpm }: Props) {
  ...
  return (
    <div className="size-full flex flex-col px-4 py-6">
      ...
      <div className="flex-1 flex gap-4 overflow-hidden">
        <TeamColumn team={teams.A} activeTurn={getTeamActiveTurn("A")} />
        <div className="flex flex-col items-center justify-center gap-2 shrink-0">
          <div className="w-px flex-1 bg-white/10" />
          <span className="text-white/30 text-xs font-bold">VS</span>
          <div className="w-px flex-1 bg-white/10" />
        </div>
        <TeamColumn team={teams.B} activeTurn={getTeamActiveTurn("B")} />
      </div>
...
function TeamColumn({ team, activeTurn }: { team: Team; activeTurn: CurrentTurnInfo | null }) {
