...
def upgrade() -> None:
    # This migration is written to be safe to (re)run against a database
    # that's in an unknown partial state — e.g. the app's own create_all()
    # already created `friendships`/`chat_messages` on a prior startup
    # (it picks up brand-new tables automatically), even though the `users`
    # columns below still need a real ALTER TABLE. Everything is guarded by
    # an inspector check so this only does the work that's actually missing.
    bind = op.get_bind()
    insp = inspect(bind)

    existing_user_columns = {c["name"] for c in insp.get_columns("users")}
    ...
    new_columns = [
        ...
        ("stripe_subscription_id", sa.String(), {"nullable": True}),
        ("premium_expires_at", sa.DateTime(timezone=True), {"nullable": True}),
        ("avatar_url", sa.String(), {"nullable": True}),
        ("tiktok_handle", sa.String(), {"nullable": True}),
        ("instagram_handle", sa.String(), {"nullable": True}),
    ...
    if "ix_users_stripe_subscription_id" not in existing_user_indexes:
    ...
    if "friendships" not in existing_tables:
        ...
        existing_friendship_indexes = {ix["name"] for ix in insp.get_indexes("friendships")}
        if "ix_friendships_requester_id" not in existing_friendship_indexes:
            op.create_index("ix_friendships_requester_id", "friendships", ["requester_id"])
        if "ix_friendships_addressee_id" not in existing_friendship_indexes:
            op.create_index("ix_friendships_addressee_id", "friendships", ["addressee_id"])
    ...
    if "chat_messages" not in existing_tables:
        ...
        existing_chat_indexes = {ix["name"] for ix in insp.get_indexes("chat_messages")}
        if "ix_chat_messages_match_id" not in existing_chat_indexes:
            op.create_index("ix_chat_messages_match_id", "chat_messages", ["match_id"])
