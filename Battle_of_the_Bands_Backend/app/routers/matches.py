        ...
        async for raw in websocket.iter_json():
            ...
            if msg_type == "join_queue":
                ...
                if room.is_full():
                    ...
                    # Start solo timeout — if room still not full, fill with NPCs
                    async def solo_timeout(r=room):
                        await asyncio.sleep(SOLO_TIMEOUT_SECS)
                        if r.status == "waiting" and not r.is_full():
                            await start_match(r)
            ...
            # ── RECORDING DONE ──────────────────────────────────────
                ...
                recording_id = raw.get("recordingId")  # set after HTTP upload
