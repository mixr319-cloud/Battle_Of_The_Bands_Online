    ...
    try:
        async for raw in websocket.iter_json():
            ...
            # ── JOIN QUEUE ──────────────────────────────────────────
                ...
                room, is_new_room = get_or_create_room(team_size, genre)
                ...
                if room.is_full():
                    ...
                    # Only spawn ONE timeout per room (when it's first created).
                    # If every joiner spawned a task, multiple start_match calls
                    # would fire and put the same player into two games.
                    async def solo_timeout(r=room):
