...
def _build_redirect_url(auth_type: str, user: User) -> str:
    """Build the post-OAuth redirect URL with all values properly URL-encoded.
    Display names / usernames can contain spaces, &, #, emoji, etc. — raw
    string concatenation would corrupt the query string."""
    params = {
        "auth": auth_type,
        "userId": user.id,
        ...
        "color": user.avatar_color.replace("#", ""),
        "level": user.level,
        "xp": user.xp,
...
async def discord_callback(code: str, db: AsyncSession = Depends(get_db)):
    ...
    return RedirectResponse(_build_redirect_url("discord", user))
...
async def google_callback(code: str, db: AsyncSession = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if token_res.status_code != 200:
            ...
            # "invalid_client", "invalid_grant") instead of a generic
            # message — that detail is what actually tells us what's
            # misconfigured. It's logged server-side via the raised
            # HTTPException, not exposed in a way that leaks secrets.
            print(f"[google_callback] token exchange failed: {token_res.status_code} {token_res.text}")
            raise HTTPException(400, f"Failed to exchange Google code: {token_res.text}")
        token_data = token_res.json()
        access_token = token_data["access_token"]
        ...
        if user_res.status_code != 200:
            print(f"[google_callback] userinfo fetch failed: {user_res.status_code} {user_res.text}")
            raise HTTPException(400, f"Failed to fetch Google user: {user_res.text}")
    ...
    return RedirectResponse(_build_redirect_url("google", user))
