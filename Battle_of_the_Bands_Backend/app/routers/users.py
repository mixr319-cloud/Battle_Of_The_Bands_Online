...
async def register_guest(body: GuestRegisterRequest, db: AsyncSession = Depends(get_db)):
    # If a guest account with this username already exists, reclaim it instead
    # of rejecting the request. Guests have no password, so the username is
    # effectively their only key — without this, a cleared localStorage or a
    # login from a second device/browser would force a brand-new UUID and
    # orphan all of that player's saved XP/level/stats permanently.
    # Non-guest (OAuth) accounts still can't be reclaimed this way.
    existing = await db.execute(select(User).where(User.username == body.username))
    existing = existing.scalar_one_or_none()
    if existing:
        if existing.auth_type == "guest":
            return user_to_dict(existing)
        raise HTTPException(400, "That name is taken by a registered account — try a different name")
