...
def get_or_create_room(team_size: int, genre: str) -> tuple[MatchRoom, bool]:
    """Returns (room, is_new). is_new is True only when a fresh room was created."""
    key = (team_size, genre)
...
async def start_match(room: MatchRoom, db=None):
    ...
    # Need at least one player per side to start
