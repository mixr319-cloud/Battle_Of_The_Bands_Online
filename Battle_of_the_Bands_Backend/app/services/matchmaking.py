...
class MatchRoom:
    def __init__(self, match_id: str, team_size: int, genre: str):
        ...
        self.loop_vote_timer_task: Optional[asyncio.Task] = None  # 30-second timeout task
    ...
    def cancel_loop_vote_timer(self):
        """Cancel any running loop-vote timeout task."""
...
async def _loop_vote_timeout(room: MatchRoom):
    ...
    if not room.loop_vote_resolved:
        ...
        kept = room.tally_loop_vote()  # partial votes still count; tie/empty → keep
...
async def start_loop_vote(room: MatchRoom, recorder_id: str, recording: Optional[dict], audio_b64: Optional[str] = None):
    ...
    room.loop_vote_timer_task = asyncio.create_task(_loop_vote_timeout(room))
...
async def advance_turn(room: MatchRoom, recording: Optional[dict] = None, db=None):
    ...
    while True:
        ...
        if current is None:
            ...
            # Start 30-second end-of-game vote timer
