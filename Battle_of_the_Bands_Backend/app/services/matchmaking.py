...
async def _force_finish_voting(room: MatchRoom, db=None):
    ...
    def find_mvp(player_ids: List[str]) -> str:
        ...
        for pid in player_ids:
            count = mvp_tally.get(pid, 0)
            if count > best_count:
                best_count = count
                best = pid
        return best  # "" if nobody received any MVP votes
    ...
    if db:
        ...
        for p in room.players:
            ...
            try:
                ...
                is_mvp = bool(mvp_a_id or mvp_b_id) and (p["id"] in (mvp_a_id, mvp_b_id))
                is_tie = (winner == "TIE")
                if is_tie:
                    earned_xp = 90 + (120 if is_mvp else 0)
                else:
