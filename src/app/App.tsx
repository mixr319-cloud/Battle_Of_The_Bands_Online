  ...
  useEffect(() => {
    const unsub = onMessage((msg) => {
      ...
      if (type === "game_start" || type === "turn_advance") {
        ...
        // The turn object includes a userId field (set by the server) so we
        // compare directly — this works even when a player covers multiple slots.
        const turn = msg.currentTurn as { teamId: "A" | "B"; playerIdx: number; userId?: string } | null;
        if (turn && profile) {
          const isMe = turn.userId
            ? turn.userId === profile.id
            : // Fallback for servers that don't send userId: search both teams
              [...teamA.players, ...teamB.players].some(
                p => p.id === profile.id && p.teamId === turn.teamId && p.playerIdx === turn.playerIdx
              );
