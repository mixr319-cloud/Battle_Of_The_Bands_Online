...
export interface Player {
  ...
  isNpc?: boolean;
  isPremium?: boolean;
  /** Profile picture URL — only set for premium members. */
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onMessage((msg) => {
      ...
      if (type === "results") {
        setVoteResult(msg.winner as "A" | "B" | "TIE");
  ...
  return (
    <div className="relative size-full overflow-hidden">
          ...
          {screen === "results" && teams && voteResult && mvpResult && (
            <motion.div key="results" {...fadeSlide} className="size-full">
              <ResultsScreen
                ...
                onViewProfile={profile?.isPremium ? (uid) => { setViewProfileUserId(uid); setProfileModalOpen(true); } : undefined}
              />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {profile && (
        <UserProfileModal
          targetUserId={viewProfileUserId}
          viewerProfile={profile}
          open={profileModalOpen}
          onClose={() => { setProfileModalOpen(false); setViewProfileUserId(null); }}
          onPatchProfile={patchProfile}
        />
      )}

      {profile && screen !== "lobby" && screen !== "finding" && matchId && (
        <GameChat
      ...
      {screen !== "game" && screen !== "voting" && screen !== "results" && (
        <motion.button
