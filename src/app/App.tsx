...
export default function App() {
  ...
  const [currentTurnA, setCurrentTurnA] = useState<{ teamId: "A" | "B"; playerIdx: number } | null>(null);
  const [currentTurnB, setCurrentTurnB] = useState<{ teamId: "A" | "B"; playerIdx: number } | null>(null);
  const [turnIdxA, setTurnIdxA] = useState(0);
  const [turnIdxB, setTurnIdxB] = useState(0);
  ...
  useEffect(() => {
    const unsub = onMessage((msg) => {
      ...
      if (type === "game_start" || type === "turn_advance") {
        ...
        const newTurnA = msg.currentTurnA as { teamId: "A" | "B"; playerIdx: number } | null;
        const newTurnB = msg.currentTurnB as { teamId: "A" | "B"; playerIdx: number } | null;
        setCurrentTurnA(newTurnA);
        setCurrentTurnB(newTurnB);
        setTurnIdxA(msg.turnIdxA as number);
        setTurnIdxB(msg.turnIdxB as number);
        ...
        // Check if it's MY turn (on MY team — the other team may be recording too)
        if (profile) {
          const myPlayer = teamA.players.find(p => p.id === profile.id)
            || teamB.players.find(p => p.id === profile.id);
          const myTurn = myPlayer?.teamId === "A" ? newTurnA : myPlayer?.teamId === "B" ? newTurnB : null;
          const isMe = !!(myPlayer && myTurn && myTurn.playerIdx === myPlayer.playerIdx);
  ...
  // The current player's own turn — derived from whichever team they're on.
  // Their team may be ahead of or behind the other team; that's expected now.
  function getMyTurn(): { teamId: "A" | "B"; playerIdx: number } | null {
    if (!teams || !profile) return null;
    const myPlayer = teams.A.players.find(p => p.id === profile.id)
      || teams.B.players.find(p => p.id === profile.id);
    if (!myPlayer || !myPlayer.teamId) return null;
    const turn = myPlayer.teamId === "A" ? currentTurnA : currentTurnB;
    if (turn && turn.playerIdx === myPlayer.playerIdx) return turn;
    return null;
  }
  ...
  async function onRecordDone(audioBlob?: Blob, waveform?: number[]) {
    ...
    if (!teams || !myTurn || !matchId || !profile) return;

    const { teamId, playerIdx } = myTurn;
  ...
  function onPlayAgain() {
    ...
    setTeams(null);
    setCurrentTurnA(null);
    setCurrentTurnB(null);
    setTurnIdxA(0);
    setTurnIdxB(0);
    setVoteResult(null);
    setVotes({ A: 0, B: 0 });
  ...
  const myTurnForRender = getMyTurn();
  const currentTeamStack = myTurnForRender
    ? recordingsRef.current.filter(r => r.teamId === myTurnForRender.teamId)
    : [];
    ...
    <div className="relative size-full overflow-hidden">
      ...
      <div className="relative size-full">
        <AnimatePresence mode="wait">
          ...
          {screen === "game" && teams && (currentTurnA || currentTurnB) && battleKey && (
            <motion.div key="game" {...fadeSlide} className="size-full">
              <GameScreen
                teams={teams}
                currentTurnA={currentTurnA}
                currentTurnB={currentTurnB}
                teamSize={teamSize}
                battleKey={battleKey}
                bpm={bpm}
              />
      ...
      <AnimatePresence>
        {showRecording && myTurnForRender && teams && battleKey && (
          <RecordingModal
            player={teams[myTurnForRender.teamId].players[myTurnForRender.playerIdx]}
            turnNumber={(myTurnForRender.teamId === "A" ? turnIdxA : turnIdxB) + 1}
