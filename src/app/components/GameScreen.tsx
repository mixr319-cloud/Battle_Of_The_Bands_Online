...
export function GameScreen({ teams, currentTurn, currentTurns, teamSize, battleKey, bpm, genre, disconnectedPlayers }: Props) {
  ...
  return (
    <div className="relative size-full flex flex-col px-4 py-6 overflow-y-auto">
      ...
      {/* Shared battle info */}
      <div className="relative z-10 flex items-center justify-center gap-3.5 mb-5 shrink-0">
        <div
          className="flex items-center gap-2 px-5 py-2.5 rounded-full"
          style={{ background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.4)", boxShadow: "0 0 20px -4px rgba(168,85,247,0.4)", backdropFilter: "blur(20px)" }}
        >
          <span className="text-xs uppercase tracking-widest font-bold" style={{ color: "#d8b4fe" }}>Key</span>
          <span className="font-bold text-sm text-white" style={{ textShadow: "0 0 10px rgba(168,85,247,0.6)" }}>{battleKey.root} {battleKey.mode}</span>
        </div>
        <div
          className="flex items-center gap-2 px-5 py-2.5 rounded-full"
          style={{ background: "rgba(34,211,238,0.12)", border: "1px solid rgba(34,211,238,0.35)", boxShadow: "0 0 20px -4px rgba(34,211,238,0.35)", backdropFilter: "blur(20px)" }}
        >
          <span className="text-xs uppercase tracking-widest font-bold" style={{ color: "#a5f3fc" }}>BPM</span>
          <span className="font-bold text-sm text-white" style={{ textShadow: "0 0 10px rgba(34,211,238,0.5)" }}>{bpm}</span>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 relative z-10">
                <div
                  className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 relative"
