...
interface Props {
  ...
  /** Seconds allowed to vote (from server, default 30) */
  timeoutSecs?: number;
}
...
export function LoopVoteModal({
  ...
  // Countdown timer — only runs for non-recorder teammates who haven't voted yet
  useEffect(() => {
    if (isRecorder || voted || result) return;
  ...
  }, [isRecorder, voted, result, timeoutSecs]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Timer bar — only for non-recorder teammates who haven't voted */}
        ...
        <div className="p-6 pb-8">
          ...
          <div className="text-center mb-5">
            ...
            {isRecorder ? (
              ...
              <p className="text-white/35 text-sm mt-0.5">Vote submitted — waiting for teammates…</p>
            ) : (
              <div className="flex items-center justify-center gap-2 mt-0.5">
                <p className="text-white/35 text-sm">Listen and cast your vote</p>
                {!result && (
                  <motion.span
                    key={secondsLeft}
                    initial={{ scale: timerUrgent ? 1.2 : 1 }}
                    animate={{ scale: 1 }}
