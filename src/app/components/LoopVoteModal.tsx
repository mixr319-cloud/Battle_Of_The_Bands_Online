...
export function LoopVoteModal({ loop, teamPlayers, teamStack, onResult }: Props) {
  // The recorder votes on their own loop — only their teammates vote
  const voters = teamPlayers.filter(p => p.name !== loop.playerName);

  // If somehow all players have the same name, fall back to full team
  const effectiveVoters = voters.length > 0 ? voters : teamPlayers;

  const [voterIdx, setVoterIdx] = useState(0);
  const [votes, setVotes] = useState<VoteValue[]>(Array(effectiveVoters.length).fill(null));
  ...
  useEffect(() => {
    ...
    if (teamStack.length > 0) {
      const blobbed = teamStack.filter(r => r.blob);
      if (blobbed.length > 0) {
        Promise.all(blobbed.map(r => r.blob!.arrayBuffer().then(b => ctx.decodeAudioData(b))))
  ...
  function castVote(value: "keep" | "drop") {
    ...
    if (voterIdx + 1 < effectiveVoters.length) {
      setVoterIdx(i => i + 1);
    } else {
      // All voted — tally
      setRevealing(true);
  ...
  const voter = effectiveVoters[voterIdx];
        ...
        <div className="p-6 pb-8">
          ...
          <div className="text-center mb-5">
            ...
            <p className="text-white/35 text-sm mt-0.5">
              Teammates listen and vote — majority rules
            </p>
          </div>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-white/40 text-xs">
                  Voter {voterIdx + 1} of {effectiveVoters.length}
                </p>
              </div>
              {/* Progress dots */}
              <div className="flex gap-1.5">
                {effectiveVoters.map((_, i) => (
