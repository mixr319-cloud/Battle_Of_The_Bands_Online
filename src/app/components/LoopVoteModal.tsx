...
interface Props {
  ...
  /** Seconds remaining before the server auto-resolves this vote */
  timeoutSecs?: number;
  /**
   * True if the server already has this player's vote on file for the
   * CURRENT loop — set on reconnect resync (see pendingLoopVote /
   * alreadyVoted in App.tsx). Without this, a teammate who already voted
   * and then refreshes/reconnects mid-vote gets the vote buttons shown to
   * them again instead of the "vote submitted" state, since this
   * component's own `voted` state always starts at false on mount.
   */
...
export function LoopVoteModal({
  ...
  timeoutSecs = 30, alreadyVoted = false,
}: Props) {
  const isRecorder = myPlayerId === recorderId;

  const [isPlaying, setIsPlaying] = useState(false);
  const [hasListened, setHasListened] = useState(alreadyVoted);
  ...
  // Countdown to when the server auto-resolves this vote (keeps the loop by
  // default if not everyone has voted by then — see LOOP_VOTE_TIMEOUT_SECS
  // on the server). Purely informational on the client: the server's own
  // timer is what actually resolves the vote, this just stops the visible
  // countdown once a result has come in so it doesn't keep ticking under
  // the reveal overlay.
  useEffect(() => {
    if (result) return;
        ...
        <div className="p-6 pb-8">
          ...
          <div className="text-center mb-5">
            <div className="flex items-center justify-center gap-2">
              ...
              {!result && (
                <span className="text-xs px-3 py-1 rounded-full font-bold tabular-nums"
                  style={{
                    background: secondsLeft <= 5 ? "rgba(248,113,113,0.15)" : "rgba(255,255,255,0.07)",
                    color: secondsLeft <= 5 ? "#fca5a5" : "rgba(255,255,255,0.4)",
                    border: `1px solid ${secondsLeft <= 5 ? "rgba(248,113,113,0.3)" : "rgba(255,255,255,0.1)"}`,
                  }}>
                  {secondsLeft}s
                </span>
              )}
