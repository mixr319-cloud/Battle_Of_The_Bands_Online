...
export function RecordingModal({
  ...
  // Stack decode loading state — true while decoding teammates' audio for the first time
  const [stackLoading, setStackLoading] = useState(!isFirstRecording);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      // Auto-play the stack so the player immediately hears what they're building on
      if (stackBuffersRef.current.length > 0) {
        ...
        // Stop auto-play after the longest buffer ends
        const maxDuration = Math.max(...stackBuffersRef.current.map(b => b.duration));
        setTimeout(() => {
            ...
            <div className="mb-6">
              <div className="rounded-2xl p-5 mb-4"
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {!stackLoading && isPlayingPreview && (
                    <span className="text-xs flex items-center gap-1" style={{ color: player.color }}>
                ...
                <button onClick={handleTogglePreview}
                  ...
                  className="w-full py-2.5 rounded-xl text-sm font-medium transition-all active:scale-95 disabled:opacity-40"
                  style={{
                  ...
                  {stackLoading ? "⟳ Loading stack…" : isPlayingPreview ? "⏸ Stop Preview" : "▶ Play Stack Again"}
                </button>
              </div>
