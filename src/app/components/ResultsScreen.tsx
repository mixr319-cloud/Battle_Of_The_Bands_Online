...
interface Props {
  ...
  onPlayAgain: () => void;
  xpSaveWarning?: boolean;
  /** Current viewer's own account id — used to skip rendering a profile link on yourself. */
  currentUserId?: string;
  /** Called with a player's id when their row/card is clicked, so the parent can open their profile. */
  onViewProfile?: (userId: string) => void;
}

export function ResultsScreen({ teams, winner, votes, mvp, recordings, onPlayAgain, xpSaveWarning, currentUserId, onViewProfile }: Props) {
  const [showDetails, setShowDetails] = useState(false);
  const [isPlaying, setIsPlaying] = useState(false);
  const isTie = winner === "TIE";
  const winTeam = isTie ? null : teams[winner];
  ...
  useEffect(() => {
    ...
    const ctx = getCtx();
    // On a tie, play all recordings; otherwise only the winning team's
    const winRecs = isTie ? recordings : recordings.filter(r => r.teamId === winner);
    Promise.all(
      winRecs.map(r =>
  ...
  // Find MVP player objects — null if no one was voted MVP
  const mvpPlayerA = mvp.A ? (teams.A.players.find(p => p.id === mvp.A) ?? null) : null;
  const mvpPlayerB = mvp.B ? (teams.B.players.find(p => p.id === mvp.B) ?? null) : null;
  const hasMvp = !!(mvpPlayerA || mvpPlayerB);
    ...
    <div className="size-full flex flex-col items-center justify-center px-6 overflow-y-auto py-8">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          {isTie ? (
            <>
              <h2 className="text-white font-black text-4xl tracking-tight">It's a Tie!</h2>
              <p className="text-white/50 text-sm mt-1">Both teams fought to a draw</p>
            </>
          ) : (
            <>
          ...
          <div className="flex justify-center gap-6 mt-4">
            {(["A", "B"] as const).map(id => (
              <div key={id} className="text-center">
                <div className="text-2xl font-black" style={{ color: isTie ? "#a855f7" : id === winner ? "#a855f7" : "rgba(255,255,255,0.3)" }}>
                  {votes[id]}
                </div>
          ...
          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden mt-3 max-w-xs mx-auto">
            {isTie ? (
              <div className="h-full w-full rounded-full" style={{ background: "linear-gradient(90deg, #a855f7, #6366f1)" }} />
            ) : (
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${(votes[winner as "A" | "B"] / Math.max(totalVotes, 1)) * 100}%` }}
          ...
          >
            ...
            {isPlaying ? (isTie ? "Playing both songs…" : "Playing winning song…") : (isTie ? "Play both songs" : "Play winning song")}
          </button>
        </div>
        ...
        {showDetails && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="rounded-2xl p-5"
              ...
              {!hasMvp ? (
                <p className="text-white/30 text-sm text-center py-2">No MVP chosen — nobody voted</p>
              ) : (
                    ...
                    if (!player) {
                      return (
                        <div key={teamId} className="rounded-xl p-3 text-center"
                          ...
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white/20 text-xl mx-auto mb-2">—</div>
                          <div className="text-white/30 text-sm">No MVP</div>
                          <div className="text-white/20 text-xs">Team {teamId}</div>
                        </div>
                      );
                    }
                    const clickable = !player.isNpc && player.id !== currentUserId && !!onViewProfile;
                    return (
                      <div
                        key={teamId}
                        onClick={clickable ? () => onViewProfile!(player.id) : undefined}
                        className={`rounded-xl p-3 text-center ${clickable ? "cursor-pointer transition-transform active:scale-95 hover:brightness-110" : ""}`}
                        style={{ background: `${player.color}11`, border: `1px solid ${player.color}33` }}>
                        <PremiumAvatarFrame isPremium={player.isPremium} rounded="rounded-xl" className="mx-auto mb-2">
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-base font-black text-white overflow-hidden"
            ...
            <div className="rounded-2xl p-5"
              ...
              {xpSaveWarning && (
                <div className="mb-4 rounded-xl px-3 py-2 text-xs text-amber-300"
                  style={{ background: "rgba(245,158,11,0.12)", border: "1px solid rgba(245,158,11,0.3)" }}>
                  ⚠ XP shown below may not have saved to your profile. We'll retry automatically — check your level after a moment.
                </div>
              )}
              <div className="space-y-3">
                {[...teams.A.players, ...teams.B.players].map(player => {
                  const isWinner = !isTie && teams[winner as "A" | "B"].players.some(p => p.id === player.id);
                  const isMvpA = mvpPlayerA ? player.id === mvpPlayerA.id : false;
                  const isMvpB = mvpPlayerB ? player.id === mvpPlayerB.id : false;
                  const isMvp = isMvpA || isMvpB;
                  const baseXp = isTie ? tieBaseXp : (isWinner ? winnerBaseXp : loserBaseXp);
                  const totalXp = baseXp + (isMvp ? mvpBonusXp : 0);
                  // The server sends the already-updated xp on the player object after awarding XP.
                  // earnedXp is the amount just awarded this match (set by the server).
                  // If earnedXp is present, player.xp is the NEW value, so we animate backwards
                  // from (xp - earned) → xp. If earnedXp is absent (old server), fall back to
                  // the old behaviour of treating player.xp as the pre-match value.
                  const earnedXp: number = player.earnedXp ?? totalXp;
                  const xpBefore = earnedXp === totalXp && !player.earnedXp
                    ? player.xp                  // fallback: xp is pre-match value
                    : player.xp - earnedXp;      // normal: xp is post-match, subtract to get start
                  ...
                  return (
                    <div
                      key={player.id}
                      onClick={clickable ? () => onViewProfile!(player.id) : undefined}
                      className={`flex items-center gap-3 ${clickable ? "cursor-pointer transition-opacity active:opacity-70 hover:opacity-90" : ""}`}
                    >
                      <PremiumAvatarFrame isPremium={player.isPremium} rounded="rounded-lg" className="shrink-0" crownSize="text-[8px]">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="h-1 rounded-full bg-white/10 overflow-hidden">
                          <motion.div
                            initial={{ width: `${Math.min((xpBefore / player.xpToNext) * 100, 100)}%` }}
                            animate={{ width: `${Math.min((xpAfter / player.xpToNext) * 100, 100)}%` }}
