  ...
  async function sendFriendRequest() {
    try {
      const res = await fetch(
        `${API_BASE}/profiles/${viewerProfile.id}/add-friend?target_id=${targetUserId}`,
        { method: "POST" }
      );
      const data = await res.json();
      setFriendStatus(data.status === "accepted" ? "accepted" : "pending");
    } catch {
      // swallow
    }
  }
                    ...
                    <div className="flex gap-2">
                      ...
                      {!isOwnProfile && (
                        <button
                          ...
                          className="flex-1 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 disabled:opacity-60"
