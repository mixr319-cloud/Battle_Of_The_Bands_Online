...
export function VotingScreen({ teams, recordings, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all(recA.map(r => r.blob.arrayBuffer().then(b => ctx.decodeAudioData(b))))
      .then(bufs => { buffersRef.current.A = bufs; });
    Promise.all(recB.map(r => r.blob.arrayBuffer().then(b => ctx.decodeAudioData(b))))
      .then(bufs => { buffersRef.current.B = bufs; });
  }, []);
