...
export function useProfile() {
  ...
  const refreshProfile = useCallback(async () => {
    ...
    try {
      ...
      // Guard against a race with login (OAuth or guest): if a *different*
      // profile got saved while this fetch was in flight, someone signed
      // into a different account in the meantime. Don't clobber it with
      // the now-stale data we started fetching for.
  ...
  // Skip this entirely if we just landed here from an OAuth redirect (the URL
  // still has ?auth=discord/google&... at this point, before App.tsx's effect
  // clears it) — that effect is about to set the real, fresh OAuth profile, and
  // this fetch would only race it using whatever profile was cached *before*
  // sign-in (e.g. a guest profile), overwriting the correct one once it resolves.
  useEffect(() => {
